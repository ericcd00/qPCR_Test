...
ct_cond_density <- ggplot(df_comb) +
    geom_density(aes(x = CP, color = Group)) +
    geom_rug(aes(x = CP, y = 0, color = Group), position = position_jitter(height = 0)) +
  theme_bw() +
  theme(plot.title = element_text(hjust = .5, vjust = 0.6, face = "bold", size = "20"),
        panel.border = element_blank(),
        panel.grid.major = element_blank(),
...
```{r echo=FALSE, fig.height = height, fig.width = 10, fig.align = "center", results='asis'}
cat("##### Violin Plot \n")
plot(ct_plot_Group)
cat('\n\n')
```
